...
// The pre-ticket flow, kept for links that are already in flight and for
// RecoveryHashRedirect, which forwards a stray Supabase landing here with the
// session still in the URL hash. New emails carry a `?t=` ticket instead and
// are handled server-side — see ./actions.ts and src/lib/agent-invite.ts.
//
// An admin-generated recovery/invite link (auth.admin.generateLink) is a
// NON-PKCE link, so Supabase redirects here with the session in the URL HASH
// (#access_token&refresh_token). We disable the client's auto URL detection
// and establish the session explicitly from the hash (or a ?code= for the PKCE
// case), which is far more reliable than relying on detectSessionInUrl.
export default function LegacyHashFlow() {
